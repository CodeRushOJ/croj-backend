...
- OpenAPI 联系信息只保留 CodeRushOJ 组织名称与项目组织链接，不再把维护者个人姓名、邮箱或个人主页发布到 Swagger 文档。
- 增加 OpenAPI 联系信息回归测试，防止个人邮箱重新进入公开 API 元数据。
...
- SMTP 增加类型化且启动时校验的连接、读取与写入超时（`100–60000ms`），并将传输层 `MailException` 转换为有界失败且保留异常堆栈，邮件基础设施异常不再把 HTTP 请求拖到网关 504。
...
- 默认 JDBC 会话时区统一为 UTC，并显式要求 Connector/J 将 `@@session.time_zone` 设为 UTC，使 MySQL `CURRENT_TIMESTAMP` 与 Java `Instant` 始终位于同一时间线，避免比赛窗口、Outbox 租约等数据库时间比较产生八小时偏移。
