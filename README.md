...
Backend 与固定 Judging consumer 的真实 ZIP 契约可通过 `scripts/verify-test-bundle-contract.sh` 验证。默认从相邻 `croj-judging-server` checkout 读取已评审的最终候选 `34d339b`；非相邻目录可设置 `JUDGING_REPOSITORY=/absolute/path/to/croj-judging-server`。门禁会分别生成 v1 与 OI/SPJ v2 artifact，再由 Judging 的 archive loader 实际解析并读取。
