...
终态枚举：`ACCEPTED`、`COMPILE_ERROR`、`WRONG_ANSWER`、`TIME_LIMIT_EXCEEDED`、`MEMORY_LIMIT_EXCEEDED`、`RUNTIME_ERROR`、`SYSTEM_ERROR`、`OUTPUT_LIMIT_EXCEEDED`。其中 OLE 持久化为 submission code `8`，不会降级成运行错误。
